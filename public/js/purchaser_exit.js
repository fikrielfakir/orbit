$(document).ready(function() {
    ...
    //Add products
    if ($('#search_product_for_purchase_exit').length > 0) {
        //Add Product
        $('#search_product_for_purchase_exit')
            .autocomplete({
                ...
                select: function(event, ui) {
                    if (ui.item.qty_available > 0) {
                        $(this).val(null);
                        purchase_exit_product_row(ui.item.variation_id);
                    } else {
    ...
    $('select#location_id').change(function() {
        if ($(this).val()) {
            $('#search_product_for_purchase_exit').removeAttr('disabled');
        } else {
            $('#search_product_for_purchase_exit').attr('disabled', 'disabled');
        }
    ...
    $(document).on('click', 'button#submit_purchase_exit_form', function(e) {
        e.preventDefault();
        //Check if product is present or not.
        if ($('table#purchase_exit_product_table tbody tr').length <= 0) {
            toastr.warning(LANG.no_products_added);
            $('input#search_product_for_purchase_exit').select();
            return false;
        }

        if ($('form#purchase_exit_form').valid()) {
            $('form#purchase_exit_form').submit();
        }
    });
    ...
    $('#purchase_exit_product_table tbody')
...
function purchase_exit_product_row(variation_id) {
    var row_index = parseInt($('#product_row_index').val());
    var location_id = $('#location_id').val();
    $.ajax({
        method: 'POST',
        url: '/purchase-exit/get_product_row',
        data: { row_index: row_index, variation_id: variation_id, location_id: location_id },
        dataType: 'html',
        success: function(result) {
            $('table#purchase_exit_product_table tbody').append(result);
            
            $('table#purchase_exit_product_table tbody tr:last').find('.expiry_datepicker').datepicker({
                autoclose: true,
                format: datepicker_date_format,
            });
...
function update_table_total() {
    var table_total = 0;
    $('table#purchase_exit_product_table tbody tr').each(function() {
    ...
    $('span#total_exit').text(__number_f(final_total));
}
