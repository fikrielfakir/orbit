    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
{
    ...
    // Handle return_amount (default to 0 if not provided)
    $uf_number = isset($product['base_unit_multiplier']) ? $product['base_unit_multiplier'] : 1;
    $return_amount = isset($input['return_amount']) ? ($uf_number ? $this->num_uf($input['return_amount']) : $input['return_amount']) : 0;

    // Calculate final_total after deducting return_amount
    $final_total = ($uf_data ? $this->num_uf($input['final_total']) : $input['final_total']) - $return_amount;
    ...
    // If pay term empty, set from contact
        ...
        foreach ($products as $product) {
            ...
            if (! empty($product['transaction_sell_lines_id'])) {
                ...
                $line = [
                    ...
                    'qty_caisse' => !empty($product['qty_caisse']) ? 
                    ($uf_data ? $this->num_uf($product['qty_caisse']) : $product['qty_caisse']) : 0,
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output = [
            ...
            $output['return_amount'] = ($transaction->return_amount != 0) ? $this->num_f($transaction->return_amount) : 0;
           
            
            
            // If return_amount is stored in payments (e.g., via is_return flag):
            // $output['total_returns'] = $this->sumReturns($transaction_id);
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            $due = $transaction->final_total - $paid_amount + $transaction->return_amount; // Subtract return_amount
...
$return_amount = $uf_number 
    ? $this->num_uf($input['return_amount'] ?? 0) 
    : ($input['return_amount'] ?? 0);
...
public function addSellReturn($input, $business_id, $user_id, $uf_number = true)
{
    ...
    if (!empty($input['transaction_date'])) {
        Log::info('Original transaction_date:', ['date' => $input['transaction_date']]);
        $sell_return_data['transaction_date'] = $uf_number 
            ? $this->uf_date($input['transaction_date'], true) 
            : $input['transaction_date'];
        Log::info('Parsed transaction_date:', ['date' => $sell_return_data['transaction_date']]);
    }
