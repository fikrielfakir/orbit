...
class ProductUtil extends Util
{
    ...
    public function getDetailsFromVariation($variation_id, $business_id, $location_id = null, $check_qty = true)
    {
        ...
        $product = $query->select(
            ...
            DB::raw('(SELECT default_purchase_price FROM variations WHERE 
                        variation_id=variations.id ORDER BY id DESC LIMIT 1) as purchased_price')
        )
            ->firstOrFail();
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        // Apply location filter based on permitted locations
        if ($permitted_locations != 'all') {
        ...
        if (!empty($filters['location_id'])) {
            ...
            // If filtering by location, hide products not available in that location
        ...
        // Filter by manufacturing products if required
        ...
        // Show manufacturing data if requested
        ...
        // Return the products based on the context (view or API)
        if ($for == 'view_product') {
            return $products->get();
        } elseif ($for == 'api') {
    ...
    public function getVariationStockDetails($business_id, $variation_id, $location_id)
    {
        $purchase_details = Variation::join('products as p', 'p.id', '=', 'variations.product_id')
            ...
            ->select(
                ...
                DB::raw("SUM(IF(t.type='purchase_exit', pl.quantity, 0)) as total_purchase_exit"),
        ...
        $output = [
            ...
            'total_purchase_exit' => $purchase_details->total_purchase_exit,
    ...
    public function getVariationStockHistory($business_id, $variation_id, $location_id)
    {
        $stock_history = Transaction::leftjoin(
            ...
            ->whereIn('transactions.type', ['sell', 'purchase', 'stock_adjustment', 'opening_stock', 'sell_transfer', 'purchase_transfer', 'production_purchase', 'purchase_return', 'purchase_exit', 'sell_return', 'production_sell'])
            ->select(
                'transactions.id as transaction_id',
                'transactions.type as transaction_type',
                'sl.quantity as sell_line_quantity',
                'pl.quantity as purchase_line_quantity',
                // 'rsl.quantity_exited as sell_exit',
                'rpl.quantity_exited as purchase_exit',
                'rsl.quantity_returned as sell_return',
                'rpl.quantity_returned as purchase_return',
                'al.quantity as stock_adjusted',
                'pl.quantity_exited as combined_purchase_exit',
        ...
        foreach ($stock_history as $stock_line) {
            ...
            if ($stock_line->transaction_type == 'sell') {
                ...
                $stock_history_array[] = array_merge($temp_array, [
                    ...
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'sell',
                    'type_label' => __('manufacturing::lang.sortie_ingredients'),
                    'ref_no' => '',
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                ]);
            ...
            } elseif ($stock_line->transaction_type == 'purchase_exit') {
                $quantity_change = -1 * ($stock_line->combined_purchase_exit + $stock_line->purchase_exit);
                $stock += $quantity_change;
                $stock_history_array[] = array_merge($temp_array, [
                    'quantity_change' => $quantity_change,
                    'stock' => $this->roundQuantity($stock),
                    'type' => 'purchase_exit',
                    'type_label' => __('lang_v1.exit'),
                    'ref_no' => $stock_line->ref_no,
                    'stock_in_second_unit' => $this->roundQuantity($stock_in_second_unit),
                ]);
    ...
    public function getVariationStockMisMatch($business_id, $variation_id, $location_id)
    {
        ...
        $stock_details = $query->select(
            ...
            DB::raw("(SELECT SUM(COALESCE(PL.quantity_exited, 0)) FROM transactions 
                    LEFT JOIN purchase_lines AS PL ON transactions.id=PL.transaction_id
                    WHERE transactions.type='purchase_exit' AND transactions.location_id=$location_id
                    AND PL.variation_id=variations.id) as total_combined_purchase_exit"),
            DB::raw("(SELECT SUM(COALESCE(PL.quantity, 0)) FROM transactions 
                    LEFT JOIN purchase_lines AS PL ON transactions.id=PL.transaction_id
        ...
        foreach ($stock_details as $index => $row) {
            ...
            $total_purchase_exit = $row->total_purchase_exit ?: 0;
            ...
            $total_stock_calculated = $total_opening_stock + $total_purchased + $total_purchase_transfered + $total_sell_return + $total_manufactured
                - ($total_sold + $total_sell_transfered + $total_adjusted + $total_purchase_return + $total_combined_purchase_return + $total_ingredients_used + $total_purchase_exit);
