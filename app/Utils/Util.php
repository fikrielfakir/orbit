...
{
    // Default format for transaction_date (includes microseconds)
    $transaction_date_format = 'Y-m-d H:i:s.u';

    // Try parsing with the transaction_date format first
    try {
        $parsed_date = \Carbon::createFromFormat($transaction_date_format, $date);
        Log::info('Date parsed with transaction_date format:', ['parsed_date' => $parsed_date]);
        return $parsed_date->format('Y-m-d H:i:s'); // Return without microseconds
    } catch (\Exception $e) {
        Log::warning('Failed to parse date with transaction_date format:', ['error' => $e->getMessage()]);
    }

    // Fallback to session-based date format
    $date_format = session('business.date_format', 'Y-m-d'); // Default to 'Y-m-d' if not set
    $mysql_format = 'Y-m-d';
    ...
    // Try parsing with the session-based format
    try {
        $parsed_date = \Carbon::createFromFormat($date_format, $date)->format($mysql_format);
        Log::info('Date parsed with session-based format:', ['parsed_date' => $parsed_date]);
        return $parsed_date;
    } catch (\Exception $e) {
        Log::error('Failed to parse date with session-based format:', ['error' => $e->getMessage()]);
        // If parsing fails, return the current date and time
        $current_date = \Carbon::now()->format($mysql_format);
        Log::info('Returning current date:', ['current_date' => $current_date]);
        return $current_date;
    }
}
    /**
    ...
    {
        if ($format == 'moroccan') {
            return $this->numToMoroccanFormat($number);
        }
    ...
    /**
     * Formats number to words in moroccan format
     *
     * @return string
     */
    public function numToMoroccanFormat(float $number)
    {
        ...
        $decimal_part = ($decimal > 0) ? ' فاصلة '.($words[$decimal / 10].' '.$words[$decimal % 10]) : '';
