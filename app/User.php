...
class User extends Authenticatable
{
    ...
    public static function forDropdownWithDelivery($business_id, $include_commission_agents = false, $check_location_permission = false, $prepend_none = false, $prepend_all = false)
    {
        // Start the query to fetch users
        $query = User::join('model_has_roles', 'model_has_roles.model_id', '=', 'users.id')  // Join model_has_roles on user_id
                    ->join('roles', 'roles.id', '=', 'model_has_roles.role_id')  // Join roles on role_id from model_has_roles
                    ->where('users.business_id', $business_id)  // Filter by business_id
                    ->where('users.user_type', 'user')  // Filter users by user type
                    ->where('roles.isDelivery', 1);  // Filter roles by isDelivery = 1
    
        // Exclude commission agents if specified
        if (!$include_commission_agents) {
            $query->where('users.is_cmmsn_agnt', 0);  // Specify 'users.is_cmmsn_agnt' as 0
        }
    
        // Check location permission if needed
        if ($check_location_permission) {
            $query->onlyPermittedLocations();  // Assuming this method filters based on location permissions
        }
    
        // Get all users based on the query
        $all_users = $query->select('users.id', DB::raw("CONCAT(COALESCE(users.surname, ''),' ',COALESCE(users.first_name, ''),' ',COALESCE(users.last_name,'')) as full_name"))
                           ->get();
    
        // Pluck the result into a key-value pair of user id and full name
        $users = $all_users->pluck('full_name', 'id');
    
        // Prepend 'none' if needed
        if ($prepend_none) {
            $users = $users->prepend(__('lang_v1.none'), '');
        }
    
        // Prepend 'all' if needed
        if ($prepend_all) {
            $users = $users->prepend(__('lang_v1.all'), '');
        }
    
        // Return the final users collection
        return $users;
    }
