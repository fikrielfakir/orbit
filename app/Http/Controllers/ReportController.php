...
public function generateStockTransferReport(Request $request)
    {
        ...
        $business_id = $request->session()->get('user.business_id');
        $statuses = $this->transactionUtil->stockTransferStatuses();
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('status', function ($row) use ($statuses) {
                    ...
                    $status_color = !empty($this->transactionUtil->statusColors[$row->status]) ? $this->transactionUtil->statusColors[$row->status] : 'bg-gray';
                    return '<span class="label ' . $status_color . '">' . $statuses[$row->status] . '</span>';
                })
                ...
                ->editColumn('shipping_charges', function ($row) {
                    return '<span class="display_currency" data-currency_symbol="true">' . $row->shipping_charges . '</span>';
                })
                ->addColumn('action', function ($row) {
                    $html = '<a href="#" data-href="' . action([\App\Http\Controllers\StockTransferController::class, 'show'], [$row->id]) . '" class="btn-modal" data-container=".view_modal"><i class="fa fa-eye" aria-hidden="true"></i></a>';
                    
                    if ($row->status != 'completed') {
                        $html .= '&nbsp;<a href="' . action([\App\Http\Controllers\StockTransferController::class, 'edit'], [$row->id]) . '" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i></a>';
                    }
                    
                    return $html;
                })
                ->editColumn('supplier', '@if(!empty($supplier_business_name)) {{$supplier_business_name}},<br>@endif {{$supplier}}')
                ->rawColumns(['final_total', 'shipping_charges', 'status', 'action', 'supplier'])
                ->make(true);
        }
        ...
        return view('report.stock_transfer_report')
            ->with(compact('business_locations', 'statuses', 'suppliers'));
    }
    /**
...
public function getproductSellReport(Request $request)
{
    ...
    if ($request->ajax()) {
        ...
        $query = TransactionSellLine::join(
            ...
            ->leftjoin('units as u', 'p.unit_id', '=', 'u.id')
            ->leftjoin('variation_location_details as vld', 'v.id', '=', 'vld.variation_id') // Join for current stock
            ->leftjoin('users as usr', 't.created_by', '=', 'usr.id') // Added join to users table with alias 'usr'
            ->where('t.business_id', $business_id)
            ->where('t.type', 'sell')
            ->where('t.status', 'final')
            ->with('transaction.payment_lines')
            ->select(
                'p.id as product_id', // Group by product_id
                'p.name as product_name',
                'p.type as product_type',
                ...
                DB::raw('SUM((transaction_sell_lines.quantity - transaction_sell_lines.quantity_returned) * transaction_sell_lines.unit_price_inc_tax) as subtotal'), // Sum subtotal correctly
                DB::raw("(SELECT SUM(vld.qty_available)  
                FROM variation_location_details as vld 
                WHERE vld.variation_id = v.id 
                AND vld.location_id = t.location_id) as current_stock"),
                DB::raw("(SELECT SUM(vld.qty_available) + SUM(transaction_sell_lines.quantity- transaction_sell_lines.quantity_returned)
                FROM variation_location_details as vld 
                WHERE vld.variation_id = v.id 
                AND vld.location_id = t.location_id
                ) as charged_value"),
                
                DB::raw("CONCAT(COALESCE(usr.surname, ''), ' ', COALESCE(usr.first_name, ''), ' ', COALESCE(usr.last_name, '')) as added_by") // Using usr alias for users table
            )
            ->groupBy('p.id');
        ...
        $added_by = $request->get('added_by', null);
        if (!empty($added_by)) {
            $query->where('t.created_by', $added_by);
        }
        ...
        ->editColumn('product_name', function ($row) {
            ...
            ->editColumn('sell_qty', function ($row) {
                return '<span data-is_quantity="true" class="sell_qty" data-orig-value="' . $row->sell_qty . '" 
                    data-unit="' . $row->unit . '" >' .
            ...
            ->addColumn('current_stock', function ($row) {
                return '<span class="current_stock" data-orig-value="' . $row->current_stock . '">' .
                    $this->transactionUtil->num_f($row->current_stock, false, null, true) . '</span> ' . $row->unit;
            })
            ->addColumn('charged_value', function ($row) {
                return '<span class="charged_value" data-orig-value="' . $row->charged_value . '">' .
                    $this->transactionUtil->num_f($row->charged_value, false, null, true) . '</span> ' . $row->unit;
            })
            ->addColumn('payment_methods', function ($row) use ($payment_types) {
            ...
            ->addColumn('added_by', function ($row) {
                return $row->added_by; // Add added_by column to the Datatable
            })
            ->editColumn('customer', '@if(!empty($supplier_business_name)) {{$supplier_business_name}},<br>@endif {{$customer}}')
            ->rawColumns(['invoice_no', 'unit_sale_price', 'subtotal', 'sell_qty', 'discount_amount', 'unit_price', 'tax', 'customer', 'payment_methods', 'current_stock', 'charged_value', 'added_by'])
            ->make(true);
    }
    ...
    // Add this to get users for the dropdown
    ...
    return view('report.product_sell_report')
        ->with(compact(
            ...
            'users' // Add the users variable here
        ));
}
    /**
    ...
    public function itemsReport()
    {
        ...
        if (request()->ajax()) {
            $query = TransactionSellLinesPurchaseLines::leftJoin('transaction_sell_lines 
                ...
                // Join the stock table (e.g., variation_location_details) and alias it as 'ps'
                ->leftJoin('variation_location_details as ps', function ($join) {
                    $join->on('ps.variation_id', '=', 'v.id')
                        ->where('ps.location_id', '=', 'purchase.location_id');
                })
                ...
                ->select(
                    ...
                    'SL.sell_line_note',
                    'PL.lot_number',
                 // Subquery to calculate current stock
        DB::raw("(SELECT SUM(vld.qty_available) 
        FROM variation_location_details as vld 
        WHERE vld.variation_id = v.id 
        AND vld.location_id = purchase.location_id) as current_stock")
    );
            ...
            return Datatables::of($query)
                ...
                ->rawColumns(['current_stock', 'subtotal', 'selling_price', 'quantity', 'purchase_price', 'sale_invoice_no', 'purchase_ref_no', 'supplier', 'customer'])
                ->make(true);
        }
