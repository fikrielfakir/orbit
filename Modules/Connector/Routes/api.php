...
Route::middleware('auth:api', 'timezone')->prefix('connector/api')->group(function () {
      ...
      // Add these stock transfer routes
    Route::resource('stock-transfers', Modules\Connector\Http\Controllers\Api\StockTransferController::class)->only([
        'index', 'store', 'show', 'update', 'destroy'
    ]);
    
        // In your routes/api.php or similar
    Route::put('stock-transfers/{id}/status', [Modules\Connector\Http\Controllers\Api\StockTransferController::class, 'updateStatus']);
    Route::resource('business-location', Modules\Connector\Http\Controllers\Api\BusinessLocationController::class)->only('index', 'show');

Route::resource('contactapi', Modules\Connector\Http\Controllers\Api\ContactController::class)->only('index', 'show', 'store', 'update', 'destroy');
