 ...
 * @bodyParam sells.*.latitude float latitude of the store Example: 40.7128
 * @bodyParam sells.*.return_amount float latitude of the store Example: 40.7128
 * @bodyParam sells.*.longitude float longitude of the store Example: 0.00
    ...
    public function store(Request $request)
{
    try {
        ...
        Log::info('Store method - Raw request data:', $requestData);
        
        $sells = $request->input('sells');
        
        // Detailed validation and logging
        if (empty($sells)) {
            Log::error('Store method - No sells data provided');
            return response()->json([
                'success' => false,
                'message' => 'No sells data provided',
                'expected_format' => [
                    'sells' => [
                        [
                            'location_id' => 1,
        ...
        if (!is_array($sells)) {
            Log::error('Store method - Sells is not an array:', ['sells_type' => gettype($sells), 'sells' => $sells]);
            return response()->json([
                'success' => false,
                'message' => 'Sells must be an array',
            ], 400);
        }
        
        Log::info('Store method - Processing sells:', ['count' => count($sells), 'sells' => $sells]);
        ...
        foreach ($sells as $index => $sell_data) {
            Log::info("Store method - Processing sell #{$index}:", $sell_data);
            
            try {
                DB::beginTransaction();
                
                // Validate individual sell data
                if (empty($sell_data) || !is_array($sell_data)) {
                    throw new \Exception("Invalid sell data at index {$index}");
                }
                
                // Check for required fields
                $requiredFields = ['location_id', 'contact_id'];
                foreach ($requiredFields as $field) {
                    if (!isset($sell_data[$field]) || empty($sell_data[$field])) {
                        throw new \Exception("Missing required field '{$field}' at sell index {$index}");
                    }
                }
                
                // Validate products array specifically
                if (!isset($sell_data['products'])) {
                    Log::error("Store method - No products key in sell data at index {$index}:", $sell_data);
                    throw new \Exception("Products array is required at sell index {$index}");
                }
                
                if (empty($sell_data['products'])) {
                    Log::error("Store method - Empty products array at index {$index}:", $sell_data);
                    throw new \Exception("Products array cannot be empty at sell index {$index}");
                }
                
                if (!is_array($sell_data['products'])) {
                    Log::error("Store method - Products is not an array at index {$index}:", [
                        'products_type' => gettype($sell_data['products']),
                        'products' => $sell_data['products']
                    ]);
                    throw new \Exception("Products must be an array at sell index {$index}");
                }
                
                // Validate each product
                foreach ($sell_data['products'] as $productIndex => $product) {
                    $requiredProductFields = ['product_id', 'variation_id', 'quantity'];
                    foreach ($requiredProductFields as $field) {
                        if (!isset($product[$field])) {
                            throw new \Exception("Missing required product field '{$field}' at sell index {$index}, product index {$productIndex}");
                        }
                    }
                }
                
                Log::info("Store method - Products validation passed for sell #{$index}:", [
                    'product_count' => count($sell_data['products'])
                ]);
                
                $sell_data['business_id'] = $user->business_id;
                $input = $this->__formatSellData($sell_data);
                
                Log::info("Store method - Formatted sell data for index {$index}:", [
                    'product_count' => count($input['products'] ?? [])
                ]);
                ...
                if (empty($input['products'])) {
                    Log::error("Store method - No products after formatting at index {$index}:", [
                        'original_products' => $sell_data['products'] ?? 'not_set',
                        'formatted_input' => $input
                    ]);
                    throw new \Exception("No products found after data formatting at sell index {$index}");
                }
                ...
                Log::info("Store method - Successfully processed sell #{$index}:", [
                ...
                DB::rollback();
                Log::emergency("Store method - Model not found at sell index {$index}:", [
                    'file' => $e->getFile(),
                    'line' => $e->getLine(),
                ...
                DB::rollback();
                Log::emergency("Store method - Exception at sell index {$index}:", [
                    'file' => $e->getFile(),
                    'line' => $e->getLine(),
                    'message' => $e->getMessage(),
                    'sell_data' => $sell_data
        ...
        DB::rollback();
        Log::emergency('Store method - General exception:', [
            'file' => $e->getFile(),
            'line' => $e->getLine(),
            'message' => $e->getMessage(),
    ...
    private function __formatSellData($data, $transaction = null)
    {
        ...
        'latitude' => $this->__getValue('latitude', $data, $transaction, null, 'latitude'),
        'longitude' => $this->__getValue('longitude', $data, $transaction, null, 'longitude'),
        'return_amount' => $this->__getValue('return_amount', $data, $transaction, 0, 'return_amount'),
    ...
    // Log::info('Formatted Data:', $formated_data); // Add this line
...
public function addSellReturn(Request $request)
{
    try {
        ...
        Log::info('Received Request Data:', $input);
        
        // Improved date format validation - accept multiple common formats
        $dateFormats = [
            'Y-m-d H:i:s.u',  // With microseconds
            'Y-m-d H:i:s',    // Standard format
            'Y-m-d H:i',      // Without seconds
            'd/m/Y H:i:s',    // Alternative format
            'd-m-Y H:i:s',    // Another alternative
            'Y/m/d H:i:s',    // Yet another format
        ];
        ...
        if (isset($input['transaction_date'])) {
            foreach ($dateFormats as $format) {
                try {
                    $parsedDate = Carbon::createFromFormat($format, $input['transaction_date']);
                    if ($parsedDate) {
                        $isValidDate = true;
                        break;
                    }
            ...
            // If none of the specific formats work, try Carbon's flexible parsing
            if (!$isValidDate) {
                try {
                    $parsedDate = Carbon::parse($input['transaction_date']);
                    $isValidDate = true;
                } catch (\Exception $e) {
                    Log::warning('Could not parse transaction_date with any format:', [
        ...
        // Validation rules with flexible date handling
        $validator = Validator::make($input, [
            'transaction_id' => 'required|integer',
            'transaction_date' => 'required|string', // Changed from specific date format to string
            ...
            'products.*.unit_price_inc_tax' => 'required|numeric',
        ]);
        
        // Add custom date validation error if needed
        if (!$isValidDate) {
            $validator->after(function ($validator) {
                $validator->errors()->add('transaction_date', 'The transaction date format is invalid.');
            });
        }
        ...
        if (!empty($input['products'])) {
            ...
            // Improved date formatting with multiple fallback options
            if (isset($input['transaction_date'])) {
                ...
                foreach ($dateFormats as $format) {
                    try {
                        $parsedDate = Carbon::createFromFormat($format, $originalDate);
                        if ($parsedDate && $parsedDate->format($format) === $originalDate) {
                            $formattedDate = $parsedDate->format('Y-m-d H:i:s');
                            Log::info('Date parsed with specific format:', [
                                'format' => $format,
                ...
                // If specific formats fail, use Carbon's flexible parsing
                if (!$formattedDate) {
                    try {
                        $parsedDate = Carbon::parse($originalDate);
                        $formattedDate = $parsedDate->format('Y-m-d H:i:s');
                        Log::info('Date parsed with flexible parsing:', [
                            'original' => $originalDate,
                            'formatted' => $formattedDate
                        ]);
                    } catch (\Exception $e) {
                        Log::error('All date parsing methods failed:', [
                            'date' => $originalDate,
                            'error' => $e->getMessage()
                        ]);
                        // Last resort: use current timestamp
                        $formattedDate = Carbon::now()->format('Y-m-d H:i:s');
                        Log::warning('Using current timestamp as fallback:', ['date' => $formattedDate]);
                    }
                }
                
                $input['transaction_date'] = $formattedDate;
            }
            
            // Retrieve the original sale to get sell_line_ids
            $originalSale = Transaction::with(['sell_lines'])->findOrFail($input['transaction_id']);
            
            // Create a mapping of product_id + variation_id to sell_line_id
            $sellLineMap = [];
            foreach ($originalSale->sell_lines as $sellLine) {
                $key = $sellLine->product_id . '_' . $sellLine->variation_id;
                $sellLineMap[$key] = $sellLine->id;
            }
            
            // Add sell_line_id to each product in the request
            foreach ($input['products'] as &$product) {
                $key = $product['product_id'] . '_' . $product['variation_id'];
                if (isset($sellLineMap[$key])) {
                    $product['sell_line_id'] = $sellLineMap[$key];
                } else {
                    throw new \Exception("Unable to find sell_line_id for product {$product['product_id']} with variation {$product['variation_id']}");
                }
            }
            
            // Log the modified products array
            Log::info('Products to Return (Modified):', $input['products']);
            
            DB::beginTransaction();
            
            // Set up the parameters for the transactionUtil->addSellReturn method
            $uf_number = true; // Assuming you want to use formatted numbers
            
            // Call the utility function with the expected parameters
            $output = $this->transactionUtil->addSellReturn($input, $business_id, $user->id, $uf_number);
            
            Log::info('Sell Return Output:', ['id' => $output->id, 'final_total' => $output->final_total]);
            ...
            throw new \Exception('No products provided for return.');
        }
    } catch (ModelNotFoundException $e) {
        DB::rollback();
        Log::error('Model Not Found Exception:', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);
        return response()->json([
            'success' => false,
        ...
        DB::rollback();
        Log::error('Exception in addSellReturn:', [
            'error' => $e->getMessage(),
            'line' => $e->getLine(),
            'file' => $e->getFile(),
    ...
    public function listSellReturn()
{
    ...
    $query = Transaction::where('business_id', $business_id)
                        ...
                        ->where('status', 'final')
                        ->with(['payment_lines', 'return_parent_sell', 'return_parent_sell.sell_lines' => function($query) {
