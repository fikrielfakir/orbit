     ...
     public function store(Request $request)
    {
        try {
            ...
            $input = $request->only([
                ...
                'city', 'state', 'country', 'address_line_1', 'address_line_2','refrige_num', 
                'customer_group_id', 'zip_code', 'contact_id', 'custom_field1', 
                'custom_field2', 'custom_field3', 'custom_field4', 'email', 
                'shipping_address', 'position', 'dob'
            ]);

            // Handle refrigerator data
            $refrigeratorData = [];
            if ($request->has('refrigerator_brand') || $request->has('refrigerator_ref')) {
                $refrigeratorData = [[
                    'marque' => $request->input('refrigerator_brand'),
                    'reference' => $request->input('refrigerator_ref')
                ]];
            }
            $input['refrigerators'] = !empty($refrigeratorData) ? json_encode($refrigeratorData) : null;
 ...
 * @bodyParam type string Type of contact (supplier, customer, both, lead) Example:customer
 * @bodyParam supplier_business_name string required* Required if type is supplier
 * @bodyParam prefix string Prefix for the name of the contact
 * @bodyParam first_name string required Name of the contact
 * @bodyParam middle_name string
...
{
    try {
        // Add validation similar to store method
        $request->validate([
            'first_name' => 'sometimes|required',
            'type' => 'sometimes|in:customer,supplier,both,lead',
        ...
        $input = $request->only([
            ...
            'city', 'state', 'country', 'landline', 'customer_group_id', 'contact_id','refrige_num', 
            'custom_field1', 'custom_field2', 'custom_field3', 'custom_field4', 
            'email', 'shipping_address', 'position', 'credit_limit'
        ]);
          ...
          // Handle refrigerator data
            if ($request->has('refrigerator_brand') || $request->has('refrigerator_ref')) {
                ...
                $input['refrigerators'] = json_encode($refrigeratorData);
            }
        ...
        $input['name'] = implode(' ', array_filter($name_array));

        DB::beginTransaction();

        // Find the contact first to check if it exists
        ...
        if ($input['type'] == 'lead') {
            ...
            // Handle credit limit if provided
     ...
     * Delete contact
     *
     * @urlParam contact required id of the contact to be deleted Example: 17
     * @response {
     *     "success": true,
     ...
     * @response 400 {
     *     "success": false,
     *     "msg": "Contact has transactions and cannot be deleted"
     * }
     */
        public function destroy($id)
{
    try {
        ...
        if ($this->contactUtil->hasTransactions($contact->id)) {
            $output = [
                'success' => false,
                'msg' => __('lang_v1.contact_has_transactions')
            ];
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'msg' => __('contact.contact_deleted_success')
            ]);

        } catch (\Exception $e) {
            DB::rollBack();
            
            // If error is about missing CrmUtil, try alternative deletion
            if (str_contains($e->getMessage(), 'Target class [Modules\Crm\Utils\CrmUtil] does not exist')) {
                \Log::info("CrmUtil missing error caught, attempting alternative deletion for contact ID: $id");
                DB::beginTransaction();
                
                try {
                    // Manual deletion without CrmUtil
                    $contact->forceDelete();
                    
                    DB::commit();
                    \Log::info("Alternative deletion succeeded for contact ID: $id");
                    return response()->json([
                        'success' => true,
                        'msg' => __('contact.contact_deleted_success')
                    ]);
                    ...
                    DB::rollBack();
                    \Log::error("Alternative deletion failed: " . $innerException->getMessage());
