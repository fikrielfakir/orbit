...
class ProductionController extends Controller
{
    ...
    public function create()
    {
        ...
        $business_locations = BusinessLocation::forDropdownhasproduction($business_id);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // استلام الـ variation_id من النموذج
            $variation_id = $request->input('variation_id');

            // تحقق من وجود variation_id في جدول mfg_recipes
            $product_id = DB::table('mfg_recipes')
                ->where('variation_id', $variation_id)
                ->value('product_id');

            // إذا تم العثور على product_id، يمكن استخدامه. إذا لم يتم العثور، يمكن معالجة الحالة
            if ($product_id) {
                // هنا يمكنك استخدام product_id كما تريد
            } else {
                // إذا لم يتم العثور على variation_id، يمكنك إظهار رسالة أو اتخاذ إجراء آخر
                return redirect()->back()->with('error', 'Variation not found.');
            }


            // تحقق من وجود product_id في جدول product_locations
            $location_id2 = DB::table('product_locations')
                ->where('product_id', $product_id)
                ->value('location_id');

            // إذا تم العثور على location_id، يمكن استخدامه. إذا لم يتم العثور، يمكن معالجة الحالة
            if ($location_id2) {
                // هنا يمكنك استخدام location_id كما تريد
            } else {
                // إذا لم يتم العثور على تطابق، يمكنك إظهار رسالة أو اتخاذ إجراء آخر
                return redirect()->back()->with('error', 'Product location not found.');
            }
            ...
            $location_id1 = $transaction_data['location_id'];

            $is_final = ! empty($request->input('finalize')) ? 1 : 0;
            $transaction_data['location_id'] = $location_id2;
            ...
            // بيانات المعاملة الثانية (purchase_exit)
            $transaction_data['type'] = 'purchase_exit'; // تغيير النوع
            $transaction_data['ref_no'] = $this->transactionUtil->generateReferenceNumber('purchase_exit', $ref_count);
            $transaction_data['demandeur'] = 'Production';
            
            $transaction1 = Transaction::create($transaction_data);
            ...
            $this->productUtil->createOrUpdatePurchaseLines($transaction1, [$purchase_line_data], $currency_details, $update_product_price);
            ...
            $transaction_sell_data = [
                ...
                'mfg_purchase_exit_id' => $transaction1->id,
                'payment_status' => 'due',
