<script type="text/javascript">
    ...
    function calculateUnitCost() {
        var total_cost = parseFloat($('#final_total').val().replace(/[^0-9.]/g, '')) || 0;
        var quantity = parseFloat($('#recipe_quantity').val().replace(/[^0-9.]/g, '')) || 0;

        console.log("Total Cost (Cleaned):", total_cost);
        console.log("Recipe Quantity (Cleaned):", quantity);
        ...
        var unit_cost = total_cost / quantity;
        
        console.log("Unit Cost Calculated:", unit_cost);

        $('#unit_cost').text(Number.isFinite(unit_cost) ? unit_cost.toFixed(2) : '0.00');
    }

    // تحديث الحساب عند تغيير أي من القيم
    $('#final_total, #recipe_quantity').on('keyup change', function() {
        calculateUnitCost();
    });
