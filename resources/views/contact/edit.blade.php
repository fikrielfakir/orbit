    ...
    <div class="modal-body">
      ...
      <div class="col-md-3">
        ...
        @endif
         <div class="col-md-6">
             <div class="form-group">
    <label for="refrigerators">Réfrigérateurs</label>
    <div id="refrigerators-container">
        @if(!empty($contact->refrigerators))
            @foreach(json_decode($contact->refrigerators, true) as $index => $refrigerator)
                <div class="refrigerator-item">
                    <input type="text" name="refrigerators[{{ $index }}][marque]" placeholder="Marque" class="form-control" value="{{ $refrigerator['marque'] }}">
                    <input type="text" name="refrigerators[{{ $index }}][reference]" placeholder="référence" class="form-control" value="{{ $refrigerator['reference'] }}">
                </div>
            @endforeach
        @else
            <div class="refrigerator-item">
                <input type="text" name="refrigerators[0][marque]" placeholder="Marque"" class="form-control">
                <input type="text" name="refrigerators[0][reference]" placeholder="référence"  class="form-control">
            </div>
        @endif
    </div>
    <button type="button" id="add-refrigerator" class="btn btn-primary">Ajouter un autre réfrigérateur</button>
</div>
        </div>
...
<script>
    ...
    var container = document.getElementById('refrigerators-container');
    var index = container.children.length;
    var newItem = document.createElement('div');
    newItem.classList.add('refrigerator-item');
    newItem.innerHTML = `
        <input type="text" name="refrigerators[${index}][marque]" placeholder="Marque" class="form-control">
        <input type="text" name="refrigerators[${index}][reference]" placeholder="référence" class="form-control">
    `;
