@php
    ...
    // Create a safe customer object to prevent null errors
    $customer = $sell->contact ?? new \stdClass();
    $customer->name = $customer->name ?? __('lang_v1.walk_in_customer');
    $customer->supplier_business_name = $customer->supplier_business_name ?? '';
    $customer->contact_address = $customer->contact_address ?? '';
    $customer->mobile = $customer->mobile ?? '';
    $customer->alternate_number = $customer->alternate_number ?? '';
    $customer->landline = $customer->landline ?? '';
  ...
  <div class="modal-content">
    <div class="modal-header">
      ...
      <h4 class="modal-title" id="modalTitle"> 
        @lang('sale.sell_details') (<b>@if($sell->type == 'sales_order') @lang('restaurant.order_no') @else @lang('sale.invoice_no') @endif :</b> {{ $sell->invoice_no }})
      </h4>
      ...
      <div class="row">
        ...
        <div class="@if(!empty($export_custom_fields)) col-sm-3 @else col-sm-4 @endif">
          ...
          @else
            ...
            @if($customer->mobile)
            <br>
                {{__('contact.mobile')}}: {{ $customer->mobile }}
            @endif
            @if($customer->alternate_number)
            <br>
                {{__('contact.alternate_contact_number')}}: {{ $customer->alternate_number }}
            @endif
            @if($customer->landline)
              <br>
                {{__('contact.landline')}}: {{ $customer->landline }}
            @endif
            @if($customer->email)
              <br>
                {{__('business.email')}}: {{ $customer->email }}
            @endif
          @endif
        </div>
        <div class="@if(!empty($export_custom_fields)) col-sm-3 @else col-sm-4 @endif">
              ...
              @foreach($sell->payment_lines as $payment_line)
                @php
                  if($payment_line->is_return == 1){
                    $total_paid -= $payment_line->amount ?? 0;
                  } else {
                    $total_paid += $payment_line->amount ?? 0;
                  }
                @endphp
                <tr>
                  ...
                  <td><span class="display_currency" data-currency_symbol="true">{{ $payment_line->amount ?? 0 }}</span></td>
                  <td>
          ...
          <div class="table-responsive">
            <table class="table bg-gray">
              <tr>
                <th>{{ __('sale.total') }}: </th>
                <td></td>
                <td><span class="display_currency pull-right" data-currency_symbol="true">{{ $sell->total_before_tax ?? 0 }}</span></td>
              </tr>
              <tr>
                <th>{{ __('sale.discount') }}:</th>
                <td><b>(-)</b></td>
                <td><div class="pull-right"><span class="display_currency" @if( $sell->discount_type == 'fixed') data-currency_symbol="true" @endif>{{ $sell->discount_amount ?? 0 }}</span> @if( $sell->discount_type == 'percentage') {{ '%'}} @endif</span></div></td>
              </tr>
              @if(in_array('types_of_service' ,$enabled_modules) && !empty($sell->packing_charge))
                <tr>
                  <th>{{ __('lang_v1.packing_charge') }}:</th>
                  <td><b>(+)</b></td>
                  <td><div class="pull-right"><span class="display_currency" @if( $sell->packing_charge_type == 'fixed') data-currency_symbol="true" @endif>{{ $sell->packing_charge ?? 0 }}</span> @if( $sell->packing_charge_type == 'percent') {{ '%'}} @endif </div></td>
                </tr>
              @endif
              @if(session('business.enable_rp') == 1 && !empty($sell->rp_redeemed) )
                <tr>
                  ...
                  <td><b>(-)</b></td>
                  <td> <span class="display_currency pull-right" data-currency_symbol="true">{{ $sell->rp_redeemed_amount ?? 0 }}</span></td>
                </tr>
              @endif
              <tr>
                ...
                <td class="text-right">
                  @if(!empty($order_taxes))
                    @foreach($order_taxes as $k => $v)
                      <strong><small>{{$k}}</small></strong> - <span class="display_currency pull-right" data-currency_symbol="true">{{ $v ?? 0 }}</span><br>
                    @endforeach
                  @else
                  0.00
                  @endif
              ...
              <tr>
                ...
                <td><b>(+)</b></td>
                <td><span class="display_currency pull-right" data-currency_symbol="true">{{ $sell->shipping_charges ?? 0 }}</span></td>
              </tr>
              ...
              @if( !empty( $sell->additional_expense_value_1 )  && !empty( $sell->additional_expense_key_1 ))
                <tr>
                  ...
                  <td><b>(+)</b></td>
                  <td><span class="display_currency pull-right" >{{ $sell->additional_expense_value_1 ?? 0 }}</span></td>
                </tr>
              @endif
              @if( !empty( $sell->additional_expense_value_2 )  && !empty( $sell->additional_expense_key_2 ))
                <tr>
                  ...
                  <td><b>(+)</b></td>
                  <td><span class="display_currency pull-right" >{{ $sell->additional_expense_value_2 ?? 0 }}</span></td>
                </tr>
              @endif
              @if( !empty( $sell->additional_expense_value_3 )  && !empty( $sell->additional_expense_key_3 ))
                <tr>
                  ...
                  <td><b>(+)</b></td>
                  <td><span class="display_currency pull-right" >{{ $sell->additional_expense_value_3 ?? 0 }}</span></td>
                </tr>
              @endif
              @if( !empty( $sell->additional_expense_value_4 ) && !empty( $sell->additional_expense_key_4 ))
                <tr>
                  ...
                  <td><b>(+)</b></td>
                  <td><span class="display_currency pull-right" >{{ $sell->additional_expense_value_4 ?? 0 }}</span></td>
                </tr>
              @endif
              <tr>
                <th>{{ __('lang_v1.round_off') }}: </th>
                <td></td>
                <td><span class="display_currency pull-right" data-currency_symbol="true">{{ $sell->round_off_amount ?? 0 }}</span></td>
              </tr>
              <tr>
                <th>{{ __('sale.total_payable') }}: </th>
                <td></td>
                <td><span class="display_currency pull-right" data-currency_symbol="true">{{ $sell->final_total ?? 0 }}</span></td>
              </tr>
              @if($sell->type != 'sales_order')
              <tr>
              ...
              <tr>
                ...
                <td>
                  ...
                  @endphp
                  <span class="display_currency pull-right" data-currency_symbol="true" >{{ ($sell->final_total ?? 0) - $total_paid }}</span></td>
              </tr>
              @endif
            </table>
          </div>
  ...
  <script>
      function initMap() {
          ...
          var marker = new google.maps.Marker({
              position: location,
              map: map,
              title: "@lang('lang_v1.transaction_location')",
              icon: {
                  url: 'https://img.icons8.com/pulsar-gradient/452/delivery-stop.png',
                  scaledSize: new google.maps.Size(40, 40)
              }
          });

          var infowindow = new google.maps.InfoWindow({
              content: `<p>@lang('sale.invoice_no'): {{ $sell->invoice_no }}</p>`
          });
      ...
      window.gm_authFailure = function() {
          document.getElementById('map').innerHTML = `
              <div class="alert alert-danger">
                  @lang("lang_v1.google_maps_load_error")
              </div>
          `;
      };
  </script>
